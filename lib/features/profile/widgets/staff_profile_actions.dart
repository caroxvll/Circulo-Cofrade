...
class _StaffProfileActionsBarState extends ConsumerState<StaffProfileActionsBar> {
  var _busy = false;
  ...
  Future<void> _suspend() async {
    ...
    setState(() => _busy = true);
    try {
      await ref.read(adminRepositoryProvider).suspendProfile(
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('@${profile.handle} suspendida')),
        );
      }
    } catch (_) {
      if (mounted) {
  ...
  Future<void> _reactivate() async {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('@${profile.handle} reactivada')),
        );
      }
    } catch (_) {
      if (mounted) {
  ...
  Widget build(BuildContext context) {
    if (!ref.watch(isStaffProvider) || widget.profile.isStaff) {
