-- Cofradeo · FIX generar jornadas SS (si el botón Generar falla)
-- Ejecutar en SQL Editor PRE. Idempotente.
-- Requisitos: ya existe is_admin_user() y (idealmente) ss_liturgical_days.
...
begin
  if auth.uid() is null or not public.is_admin_user(auth.uid()) then
    raise exception 'Solo admin puede generar jornadas (profiles.role = admin)';
  end if;
